...
from src.scrapers.live_category_scraper import LiveScraper
...
class ScraperGUI:
    ...
    def init_ui(self):
        ...
        # 分类
        ttk.Label(main_frame, text="分类：", style="Title.TLabel").pack(anchor="w")
        self.category_var = tk.StringVar(value=self.runtime_data.get("category"))
        self.category_entry = ttk.Entry(main_frame, textvariable=self.category_var)
    ...
    def handle_confirm(self):
        ...
        if not browser_path or not category or not limit:
    ...
    def handle_clear(self):
        ...
        self.category_var.set(self.runtime_data.get("category"))
...
def run_task(browser_path, category, limit_int):
    browser_manager = BrowserManager(browser_path)
    live_scraper = LiveScraper(browser_manager)
    live_scraper.category_task(category, limit_int)
