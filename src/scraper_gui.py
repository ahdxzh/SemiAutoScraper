...
from src.scrapers.pgy.pgy_keywords_scraper import PgyKeywordsSearchScraper
...
class ScraperGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.browser_entry = None
        self.keywords_var = None
        self.keywords_entry = None
        self.limit_start_var = None
        self.limit_start_entry = None
        self.limit_end_var = None
        self.limit_end_entry = None
    ...
    def setup_ui_config(self):
        ...
        self.fonts = {"default": default_font, "title": title_font, "small": (default_font[0], 9)}
    ...
    def init_ui(self):
        ...
        # 平台选择
        ttk.Label(main_frame, text="选择平台：", style="Title.TLabel").pack(anchor="w")
        self.platform_var = tk.StringVar(value=self.runtime_data.get("platform", "pgy"))
        platform_frame = ttk.Frame(main_frame)
        platform_frame.pack(fill=tk.X, pady=(0, 10))
        ttk.Radiobutton(platform_frame, text="蒲公英", variable=self.platform_var, value="pgy").pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(platform_frame, text="星图", variable=self.platform_var, value="xingtu").pack(side=tk.LEFT, padx=10)
        ...
        ttk.Entry(limit_frame, textvariable=self.limit_start_var, width=10).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Label(limit_frame, text="至").pack(side=tk.LEFT, padx=5)
        ttk.Entry(limit_frame, textvariable=self.limit_end_var, width=10).pack(side=tk.LEFT, padx=(5, 0))

        # 状态
        self.status_var = tk.StringVar(value="就绪")
        ttk.Label(main_frame, textvariable=self.status_var, font=self.fonts["small"], foreground="#666").pack(anchor="w")

        # 按钮
        btn_frame = ttk.Frame(main_frame)
        btn_frame.pack(fill=tk.X, pady=5)
        ttk.Button(btn_frame, text="确认 (Enter)", command=self.handle_confirm, width=15).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="清空 (Esc)", command=self.handle_clear, width=15).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="退出", command=self.handle_exit, width=15).pack(side=tk.LEFT, padx=10)
    ...
    def handle_confirm(self):
        ...
        platform_choice = self.platform_var.get().strip()

        if not browser_path or not keywords or not limit_start or not limit_end:
            self._show_queue_message("warning", "提示", "请填写所有参数！")
            return
        ...
        try:
            ...
            if start_int > end_int:
                self._show_queue_message("warning", "提示", "起始值不能大于结束值！")
                return
            if start_int <= 0 or end_int <= 0:
                self._show_queue_message("warning", "提示", "数量必须为正整数！")
                return
        except ValueError:
            self._show_queue_message("warning", "提示", "数量必须为整数！")
            return
        ...
        # 根据平台执行任务
        if platform_choice == "pgy":
            self.executor.submit(run_pgy_task, browser_path, keywords, limit_range)
        else:
            self.executor.submit(run_xingtu_task, browser_path, keywords, limit_range)

        self._show_queue_message(
            "info",
            "开始执行",
            f"平台：{'蒲公英' if platform_choice == 'pgy' else '星图'}\n关键词：{keywords}\n计划爬取数量：{limit_range.start} - {limit_range.end}"
        )

    def handle_clear(self):
        for key, var in [
            ("browser_path", self.browser_var),
            ("keywords", self.keywords_var),
            ("limit_start", self.limit_start_var),
            ("limit_end", self.limit_end_var),
            ("platform", self.platform_var)
        ]:
            var.set(self.runtime_data.get(key))
    ...
    def _show_queue_message(self, msg_type, title, content):
        request_id = str(uuid.uuid4())
        request_queue.put((msg_type, request_id, title, content))
        ...
        while not request_queue.empty():
            ...
            try:
                if msg_type == "wait":
                    ...
                    result_queue.put((request_id, choice))
...
def run_pgy_task(browser_path, keywords, limit_range):
    browser_manager = BrowserManager(browser_path)
    scraper = PgyKeywordsSearchScraper(browser_manager)
    scraper.run(keywords, limit_range)


def run_xingtu_task(browser_path, keywords, limit_range):
    browser_manager = BrowserManager(browser_path)
    scraper = XingtuKeywordsSearchScraper(browser_manager)
    scraper.run(keywords, limit_range)
